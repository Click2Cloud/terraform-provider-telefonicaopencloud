		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCTSTrackerV1Exists("telefonicaopencloud_cts_tracker_v1.tracker_v1", &tracker),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_cts_tracker_v1.tracker_v1", "bucket_name", "tf-test-bucket"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_cts_tracker_v1.tracker_v1", "file_prefix_name", "yO8Q"),
				),
			},
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCTSTrackerV1Exists("telefonicaopencloud_cts_tracker_v1.tracker_v1", &tracker),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_cts_tracker_v1.tracker_v1", "bucket_name", "tf-test-bucket1"),
					resource.TestCheckResourceAttr(
						"telefonicaopencloud_cts_tracker_v1.tracker_v1", "file_prefix_name", "yO8Q1"),
				),
			},
		},
	})
...
func testAccCheckCTSTrackerV1Destroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "telefonicaopencloud_cts_tracker_v1" {
			continue
		}
